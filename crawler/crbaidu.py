...
class crbaidu:
    def __init__(self):
        ...
        self.__make_cookies()
        self.__driver.quit()
    def baidusearch(self,subcode,searchkey = ''):
        '''
        输入关键字，启动百度搜索
        :param searchkey:查询条件
        :return:[csdnurl,score,subcode]
        '''
        ...
        csdndata = self.__processbaidudata(r.text)
        if csdndata:
            csdndata.append(subcode)
        return csdndata
    ...
    def __processcsdndata(self,baiduurl,headers):
        ...
        score = soup.find('div',class_='dl_operate clearfix').find('em').get_text()
        if re.match('\d+',score):
            score = int(score)
...
if __name__ == '__main__':
    baidu = crbaidu()
    best = baidu.baidusearch('4175538','Flash ActionScript 3.0动画高级教程 ')
    print(best)
